#!/usr/bin/env python3
"""Google Workspace API CLI for DASHENG Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing DASHENG-facing JSON contract and falls back to the Python client
...
from _dasheng_home import get_dasheng_home
...
def _gws_binary() -> str | None:
    override = os.getenv("DASHENG_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for DASHENG Agent")
