...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("DASHENG_HOME", os.path.expanduser("~/.dasheng")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("DASHENG_HOME", Path.home() / ".dasheng")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# DASHENG config paths
# ═══════════════════════════════════════════════════════════════════

DASHENG_HOME = Path(os.getenv("DASHENG_HOME", Path.home() / ".dasheng"))
CONFIG_PATH = DASHENG_HOME / "config.yaml"
PREFILL_PATH = DASHENG_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/DASHENG models — already uncensored, just needs clean prompt
    "dasheng": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "dasheng" in model_lower or "nous" in model_lower:
        return "dasheng"
...
def _get_current_model() -> tuple:
    """Read current model and provider from DASHENG config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.dasheng/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart DASHENG for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart DASHENG for changes to take effect.")
