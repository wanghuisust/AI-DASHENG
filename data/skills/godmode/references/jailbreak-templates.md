...
## 5. GODMODE FAST — DASHENG 4 405B (Zero Refusal)

**Model:** `nousresearch/dasheng-4-405b`
**Strategy:** DASHENG models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with DASHENG Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.dasheng/config.yaml`:
