...
metadata:
  dasheng:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, dasheng-video]
...
```
DASHENG Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${DASHENG_HOME:-$HOME/.dasheng}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to DASHENG config (if missing)
...
3. **Restart DASHENG session** to pick up the new MCP server
