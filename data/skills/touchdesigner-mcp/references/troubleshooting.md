...
## 5. DASHENG Configuration

### Config location

`$DASHENG_HOME/config.yaml` (defaults to `~/.dasheng/config.yaml` when `DASHENG_HOME` is unset)
...
Restart the DASHENG session for changes to take effect. The MCP connection is
